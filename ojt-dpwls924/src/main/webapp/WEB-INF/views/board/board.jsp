...
<div class="table-form-wrap">  
    <form name="searchForm" action=${popupYn eq 'Y' ? "/board/boardList/popup" : "/board/boardList"} method="get">
    <table class="table-form">
        ...
        <tbody>
            <tr>
                ...
                <th>작성자</th>
                <td class="tl form-100p">
                    <input type="text" name="writer" value="<c:out value='${paramMap.writer}'/>" />
                </td>
            </tr>
            <tr>
                <th>내용</th>
...
<div class="table-list-wrap">
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${popupYn ne 'Y'}">
                                    <a href="/board/boardFormMod?boardArtclSeq=${board.boardArtclSeq}&searchData=<core:param encode="true" />"><c:out value='${board.boardArtclTitle}'/></a>
                            </c:when>
                            <c:otherwise>
    ...
    <div class="table-footer">
        ...
        <c:if test="${popupYn ne 'Y' }"></c:if>
        <div class="btn-wrap">
            <button type="button" class="btn btn-primary" onclick="location.href='/board/boardFormReg';">등록</button>
        </div>
    </div>
</div>
<%-- 목록 영역 종료 --%>
...
function selectBoard(index) {
    ...
    if(typeof(callbackFn) == "function"){
        callbackFn(data);
    } else if (typeof(opener.callbackBoardPopup) == "function") {
        opener.callbackBoardPopup(data);
    }
    window.close();
}
