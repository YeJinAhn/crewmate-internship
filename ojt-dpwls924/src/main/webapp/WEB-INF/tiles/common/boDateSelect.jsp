...
<c:if test="${empty strtDt}">
	<c:set var="strtDt">strtDt</c:set>
</c:if>

<c:if test="${empty endDt}">
	<c:set var="endDt">endDt</c:set>
</c:if>
<c:if test="${empty strtDtVal}">
    <c:set var="strtDtVal">${paramMap.strtDt}</c:set>
</c:if>
...
<div class="day" >
	<div class="datepicker-wrap">
		<input type="text" name="${strtDt}" class="datepicker date-from" onkeydown="return false;" value="<core:mask pattern="####.##.##"><c:out value="${strtDtVal}" /></core:mask>" />
	</div>
	~
	<div class="datepicker-wrap">
